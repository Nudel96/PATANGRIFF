      ...
      {#each tools as tool}
        <div class="military-card group relative">
          ...
          <div class="absolute -top-3 -right-3">
            <Badge class="{tool.status === 'Available' ? 'bg-primary/10 text-primary border-primary/20' : 'bg-muted text-muted-foreground border-border'}">
              {tool.status}
            </Badge>
          </div>
            ...
            <ul class="space-y-2">
              {#each tool.features as feature}
                <li class="flex items-center space-x-2">
                  <div class="w-1.5 h-1.5 bg-{tool.color} rounded-full" />
